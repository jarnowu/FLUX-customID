...
from src.utils.insightface_package import FaceAnalysis2, analyze_faces
import cv2
...
class ProjPlusModel(torch.nn.Module):
    def __init__(self, cross_attention_dim=768, id_embeddings_dim=512, clip_embeddings_dim=1280, num_tokens=4, output_dim=3072):
        super().__init__()
...
class CustomIDModel:
    def __init__(self, sd_pipe, trained_ckpt, device, dtype, num_tokens=4, image_encoder_path="pretrained_ckpt/openclip-vit-h-14"):
        self.device = device
        self.dtype = dtype
    ...
    def set_id_adapter(self):
        ...
        for name in self.pipe.transformer.attn_processors.keys():
            if name.startswith("transformer_blocks"):
                attn_procs[name] = CATRefFluxAttnProcessor2_0(self.pipe.transformer.config.num_attention_heads * self.pipe.transformer.config.attention_head_dim, 
